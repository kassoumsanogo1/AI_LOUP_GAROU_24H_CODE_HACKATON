...
# Clé API à mettre
# utiliser une clé globale key
headers = {"Authorization": "Bearer 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"}
...
class Game:
    ...
    def introduce_players(self):
        for player in self.players:
            self.rendering_engine.render_system_message(
                f'Player number {player.player_number} is named {{ref_players[0]}}, and he have the {{ref_cards[0]}} card.',
    ...
    def night_werewolf(self):
        ...
        if len(werewolf_players) == 0:
            ...
            self.rendering_engine.render_system_message(
                'There is one werewolf in play, {ref_players[0]}. The werewolf randomly viewed the middle card: {ref_cards[0]}.',
    ...
    def night_seer(self):
        ...
        if len(seer_players) == 0:
            ...
            self.rendering_engine.render_system_message(
                'There is one seer in play, {ref_players[0]}. They are thinking about their action.'
    ...
    def get_player_names(self, player_count):
        name_options = ['Kassoum', 'Alexia', 'Andrei', 'Cristina', 'Dragos', 'Dracula', 'Carel', 'Ileana', 'Kraven',
                        'Larisa', 'Lucian', 'Marius', 'Michael', 'Mircea', 'Radu', 'Anas', 'Selene', 'Stefan', 'Viktor',
                        'Vladimir']
        if (self.real_player_name != ""):
...
# Choisir le nombre de joueur ICI !!!
@click.command()
@click.option('--player-count', type=int, default=4, help='Number of players')
@click.option('--discussion-depth', type=int, default=8, help='Number of discussion rounds')
...
def play_game(player_count, discussion_depth, render_markdown):
    ...
    if (input_text == 'yes'):
        ...
        game = Game(player_count=player_count, discussion_depth=discussion_depth, render_markdown=render_markdown,
                    real_player_name=real_player_name)
    else:
        game = Game(player_count=player_count, discussion_depth=discussion_depth, render_markdown=render_markdown)
    game.play()
